...
var rcaDraftDuration = prometheus.NewHistogram(
	prometheus.HistogramOpts{
		Name:    "remediator_rca_draft_duration_seconds",
		Help:    "RCA copilot draft duration.",
		Buckets: []float64{1, 2.5, 5, 10, 20, 40, 80, 160},
	},
)

func init() { prometheus.MustRegister(rcaDraftsTotal, rcaDraftDuration) }
...
func draftRCA(alert Alert, action, verification string) {
	if copilot == nil || !copilot.Enabled() {
		return
	}
	start := time.Now()
	defer func() { rcaDraftDuration.Observe(time.Since(start).Seconds()) }()
	...
	draft, err := copilot.Draft(ctx, inc)
	if err != nil {
